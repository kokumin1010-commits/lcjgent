...
describe("SalesDash daily report LINE receiver", () => {
  ...
  it("uses a signed 202-accepted endpoint and a single default-off target group", () => {
    ...
    expect(bridgeSource).toContain("configuredNameMatchCount: nameMatches.length");
    expect(bridgeSource).toContain("activeConfiguredNameMatchCount");
    expect(bridgeSource).toContain("lastRolloutOutcome");
    expect(bridgeSource).not.toContain("targetGroupName,");
    ...
    expect(bridgeSource).toContain("groups.length !== 1");
