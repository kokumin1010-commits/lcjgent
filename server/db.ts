...
/** Active HR staff safe for schedule selectors; source rows remain untouched. */
export async function getScheduleStaffCandidates() {
  const active = await getActiveStaff();
  return filterStaffScheduleCandidates(active);
}
...
// Update LINE group active status with deterministic webhook ordering.
export async function updateLineGroupActive(
  lineGroupId: string,
  isActive: boolean,
  lifecycle?: {
  ...
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available while updating LINE group state");
  }

  const eventTimestamp = lifecycle?.eventTimestamp ?? Date.now();
  const eventId = (
    lifecycle?.eventId?.trim() ||
    `local:${eventTimestamp}:${isActive ? "active" : "inactive"}`
  ).slice(0, 64);
  ...
  return await db.transaction(async (tx) => {
    ...
    const current = lifecycleRows[0]
      ? {
          eventTimestamp: lifecycleRows[0].lastEventAt,
          eventId: lifecycleRows[0].lastEventId,
        }
      : null;
    ...
    if (lifecycleRows[0]) {
      await tx
        .update(lineGroupLifecycleStates)
        .set({
...
export async function ensureLineGroupLifecycleStorage(): Promise<void> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available while ensuring LINE group lifecycle storage");
  }
...
export async function getLiverRankings(month: string, agencyId?: number | null) {
  ...
  const salesRanking = await db
    ...
    .orderBy(sql`SUM(COALESCE(${brandLivestreams.manualSalesAmount}, ${brandLivestreams.salesAmount}, ${brandLivestreams.gmv}, 0)) DESC`);
  ...
  const cumulativeData = await db
    .select({
      liverId: brandLivestreams.liverId,
      cumulativeSales: sql<number>`COALESCE(SUM(COALESCE(${brandLivestreams.manualSalesAmount}, ${brandLivestreams.salesAmount}, ${brandLivestreams.gmv}, 0)), 0)`,
...
export async function getSubcategories(parentId: number) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(mallCategories)
    .where(eq(mallCategories.parentId, parentId))
    .orderBy(asc(mallCategories.sortOrder), asc(mallCategories.name));
}

// バリアント一覧取得
export async function getMallProductVariants(productId: number) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(mallProductVariants)
    .where(eq(mallProductVariants.productId, productId))
    .orderBy(asc(mallProductVariants.sortOrder));
}

// バリアント作成
export async function createMallProductVariant(data: InsertMallProductVariant) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(mallProductVariants).values(data);
  return result;
}

// バリアント更新
export async function updateMallProductVariant(id: number, data: Partial<InsertMallProductVariant>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(mallProductVariants).set(data).where(eq(mallProductVariants.id, id));
}

// バリアント削除
export async function deleteMallProductVariant(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(mallProductVariants).where(eq(mallProductVariants.id, id));
}

// 商品の全バリアント削除
export async function deleteAllMallProductVariants(productId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(mallProductVariants).where(eq(mallProductVariants.productId, productId));
}
...
export async function getDailyLiverBreakdown(date: string, agencyId?: number | null) {
  ...
  const result = await db
    ...
    .groupBy(brandLivestreams.liverId, livers.name, livers.avatarUrl)
    .orderBy(desc(sql`COALESCE(SUM(COALESCE(${brandLivestreams.manualSalesAmount}, ${brandLivestreams.salesAmount}, ${brandLivestreams.gmv}, 0)), 0)`));
...
export async function getLiverBrandPerformance(liverId: number, month?: string) {
  ...
  const performance = await db
    .select({
      ...
      totalDuration: sql<number>`COALESCE(SUM(${brandLivestreams.duration}), 0)`,
      totalLivestreams: sql<number>`COUNT(*)`,
      avgSalesPerStream: sql<number>`COALESCE(AVG(COALESCE(${brandLivestreams.manualSalesAmount}, ${brandLivestreams.salesAmount}, ${brandLivestreams.gmv}, 0)), 0)`,
    })
    .from(brandLivestreams)
    .leftJoin(brands, eq(brandLivestreams.brandId, brands.id))
    .where(and(...conditions))
...
export async function getTiktokTapLiveSummary(brandId: number = 0, month?: string) {
  ...
  const result = await db.select({
    ...
    totalCreatorCommission: sql<number>`COALESCE(SUM(COALESCE(actualCreatorCommission, estimatedCreatorCommission)), 0)`,
    totalActualPartnerCommission: sql<number>`COALESCE(SUM(actualPartnerCommission), 0)`,
    totalSalesCount: sql<number>`COALESCE(SUM(salesCount), 0)`,
...
// ============================================================
// Referral Bonus Offer Functions
// ============================================================

/** 確変チャンス完了後に招待ボーナスオファーを発行 */
export async function createReferralBonusOffer(lineUserId: number, campaignId: number, bonusPoints = 500) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24時間後
  const result = await db.insert(referralBonusOffers).values({
    lineUserId,
    campaignId,
    bonusPoints,
    status: "active",
  ...
  return { id: result[0].insertId, bonusPoints, expiresAt };
}

/** ユーザーのアクティブなボーナスオファーを取得 */
export async function getActiveReferralBonusOffer(lineUserId: number) {
  const db = await getDb();
  if (!db) return null;
  const now = new Date();
  const results = await db.select().from(referralBonusOffers)
    .where(and(
      eq(referralBonusOffers.lineUserId, lineUserId),
      eq(referralBonusOffers.status, "active"),
      gt(referralBonusOffers.expiresAt, now)
    ))
    .orderBy(desc(referralBonusOffers.createdAt))
    .limit(1);
  return results.length > 0 ? results[0] : null;
}

/** ボーナスオファーをclaimedに更新（招待成功時） */
export async function claimReferralBonusOffer(offerId: number, referralId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(referralBonusOffers).set({
    status: "claimed",
    claimedAt: new Date(),
    triggeredByReferralId: referralId,
  }).where(eq(referralBonusOffers.id, offerId));
}

/** 期限切れオファーをexpiredに更新（バッチ処理用） */
export async function expireReferralBonusOffers() {
  const db = await getDb();
  if (!db) return;
  const now = new Date();
  await db.update(referralBonusOffers).set({ status: "expired" })
    .where(and(
      eq(referralBonusOffers.status, "active"),
      lte(referralBonusOffers.expiresAt, now)
    ));
}
...
// ============================================================
// Livestream Promotions (単品割引プロモーション)
// ============================================================

export async function createLivestreamPromotion(data: InsertLivestreamPromotion) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(livestreamPromotions).values(data);
  return result;
}

export async function getLivestreamPromotionsByLivestreamId(livestreamId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.select().from(livestreamPromotions)
    .where(eq(livestreamPromotions.livestreamId, livestreamId))
    .orderBy(asc(livestreamPromotions.sortOrder));
}

export async function deleteLivestreamPromotionsByLivestreamId(livestreamId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getLiverBrandDurationStats(liverId: number, yearMonth?: string) {
  ...
  // ===== CSV売上データの集計（商品GMVのブランド別集計 + 重複排除） =====
  // 商品CSVのGMVをブランド名マッチングで集計（参考値）
  ...
  if (livestreamIds.length > 0) {
    // バッチで商品データを取得
    const batchSize = 100;
    let allProducts: { livestreamId: number; productName: string | null; gmv: number | null; grossRevenue: number | null; livestreamDate?: string | null }[] = [];
    for (let i = 0; i < livestreamIds.length; i += batchSize) {
      const batch = livestreamIds.slice(i, i + batchSize);
      const products = await db
        .select({
    ...
    // 重複排除: 同一商品名+同一GMVの組み合わせが複数配信に存在する場合、1回だけカウント
    const seenProducts = new Set<string>();
    const deduplicatedProducts: typeof allProducts = [];
    for (const p of allProducts) {
      const key = `${(p.productName || '').trim()}|${Number(p.gmv || 0)}|${Number(p.grossRevenue || 0)}`;
  ...
  // Step 5: CSVで売上があるのに配信ブランドとして入力されていないブランドを追加
  const unmatchedBrands = Array.from(brandSalesMap.values())
    .filter(s => s.totalGmv > 0)
    .sort((a, b) => b.totalGmv - a.totalGmv)
    .map(s => ({
      brandId: s.brandIds[0],
      brandIds: s.brandIds,
...
// ============================================================
// Master Set Suggestions DB Functions
// ============================================================

/**
 * Get all active master set suggestions with their items
 */
export async function getAllMasterSetSuggestions(status?: string) {
  const db = await getDb();
  if (!db) return [];
  
  const conditions = status ? eq(masterSetSuggestions.status, status) : undefined;
  
  const suggestions = await db
    .select()
    .from(masterSetSuggestions)
    .where(conditions)
    .orderBy(desc(masterSetSuggestions.priority), desc(masterSetSuggestions.createdAt));
  
  const suggestionsWithItems = await Promise.all(suggestions.map(async (s) => {
    const items = await db!.select().from(masterSetSuggestionItems)
      .where(eq(masterSetSuggestionItems.suggestionId, s.id))
      .orderBy(asc(masterSetSuggestionItems.sortOrder));
    return { ...s, items };
  }));
...
/**
 * Get active suggestions for liver mypage (only active + within valid period)
 */
export async function getActiveMasterSetSuggestionsForLiver() {
  const db = await getDb();
  if (!db) return [];
  ...
  const suggestions = await db
    ...
    .where(
      and(
        eq(masterSetSuggestions.status, "active"),
        or(
          isNull(masterSetSuggestions.validFrom),
          lte(masterSetSuggestions.validFrom, now)
        ),
        or(
          isNull(masterSetSuggestions.validUntil),
          gte(masterSetSuggestions.validUntil, now)
        )
      )
    )
    .orderBy(desc(masterSetSuggestions.priority), desc(masterSetSuggestions.createdAt));
...
/**
 * Create a master set suggestion
 */
export async function createMasterSetSuggestion(data: InsertMasterSetSuggestion) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
  
  const result = await db.insert(masterSetSuggestions).values(data);
  const insertId = result[0].insertId;
  return { id: insertId, ...data };
}

/**
 * Create master set suggestion items
 */
export async function createMasterSetSuggestionItems(items: InsertMasterSetSuggestionItem[]) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
...
export async function updateMasterSetSuggestion(id: number, data: Partial<InsertMasterSetSuggestion>) {
  ...
  await db.update(masterSetSuggestions).set(data).where(eq(masterSetSuggestions.id, id));
}

/**
 * Delete a master set suggestion and its items
 */
export async function deleteMasterSetSuggestion(id: number) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
  
  await db.delete(masterSetSuggestionItems).where(eq(masterSetSuggestionItems.suggestionId, id));
  await db.delete(masterSetSuggestions).where(eq(masterSetSuggestions.id, id));
}

/**
 * Record a liver adopting a master set suggestion
 */
export async function createMasterSetAdoption(data: InsertMasterSetAdoption) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
  
  const result = await db.insert(masterSetAdoptions).values(data);
  
  // Increment adoption count
  await db.update(masterSetSuggestions)
    .set({ adoptionCount: sql`${masterSetSuggestions.adoptionCount} + 1` })
    .where(eq(masterSetSuggestions.id, data.suggestionId));
  
  return { id: result[0].insertId, ...data };
}

/**
 * Get adoptions for a specific liver
 */
export async function getLiverAdoptions(liverId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  return await db
    ...
    .where(eq(masterSetAdoptions.liverId, liverId))
    .orderBy(desc(masterSetAdoptions.adoptedAt));
}

/**
 * Get all adoptions (for admin view)
 */
...
export async function getBrandAdReportsByBrandId(brandId: number) {
  ...
  return db.select()
    ...
    .orderBy(desc(brandAdReports.periodEnd));
}
...
// ============================================================
// Brand Ad Email Recipients CRUD
// ============================================================
...
// ============================================================
// Auto-migration for brand_ad_reports and brand_ad_email_recipients
// ============================================================
  ...
  try {
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS brand_ad_reports (
        ...
        createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX idx_brand_ad_reports_brandId (brandId),
        INDEX idx_brand_ad_reports_period (periodStart, periodEnd)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS brand_ad_email_recipients (
        id INT AUTO_INCREMENT PRIMARY KEY,
        brandId INT NOT NULL,
        ...
        createdAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        INDEX idx_brand_ad_email_recipients_brandId (brandId)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
    `);
    console.log("[Migration] brand_ad_reports and brand_ad_email_recipients tables ensured");
  } catch (error: any) {
    if (!error.message?.includes("already exists")) {
      console.error("[Migration] Error ensuring brand_ad_reports tables:", error);
    }
  }
}
...
export async function getAllLiverDailySales(month: string, agencyId?: number | null) {
  ...
  const rows = await db
    .select({
      ...
      sales: sql<number>`COALESCE(SUM(COALESCE(${brandLivestreams.manualSalesAmount}, ${brandLivestreams.salesAmount}, ${brandLivestreams.gmv}, 0)), 0)`,
    })
    .from(brandLivestreams)
    .leftJoin(livers, eq(brandLivestreams.liverId, livers.id))
    .where(and(...conditions))
...
// =====================================================
// All Liver Brand Efficiency (全ライバーのブランド別時間単価)
// =====================================================
/**
 * Get brand efficiency (hourly rate = sales / hours) for ALL livers in a given month.
 * Used for the /livers page efficiency alerts and recommendations.
 * Returns: Record<liverId, { brandId, brandName, totalHours, csvGmv, hourlyRate }[]>
 */
export async function getAllLiverBrandEfficiency(month: string, agencyId?: number | null) {
  const db = await getDb();
  if (!db) return {};
  const { startDate, endDate } = getJSTMonthRange(month);
  const agencyFilter = agencyId === null 
  ...
  const activeLivers = await db
    .select({
      liverId: brandLivestreams.liverId,
      liverName: sql<string>`COALESCE(${livers.name}, MAX(${brandLivestreams.streamerName}))`,
    })
    .from(brandLivestreams)
    .leftJoin(livers, eq(brandLivestreams.liverId, livers.id))
    .where(and(...conditions))
  ...
  for (const product of allProducts) {
    // Find which liver owns this livestream
    for (const [liverId, lsIds] of livestreamIdsByLiver.entries()) {
      if (lsIds.includes(product.livestreamId)) {
        if (!productsByLiver.has(liverId)) productsByLiver.set(liverId, []);
        productsByLiver.get(liverId)!.push(product);
        break;
      }
    }
  }
  ...
  for (const [liverId, entries] of durationByLiver.entries()) {
    ...
    for (const [normalizedName, data] of byNameMap.entries()) {
      ...
      const hourlyRate = totalHours > 0 ? Math.round(csvGmv / totalHours) : 0;
      liverResult.push({
        brandId: data.brandId,
        brandName: data.brandName,
        brandNameJa: data.brandNameJa,
...
export async function createLeadCollectionHistory(data: InsertLeadCollectionHistory) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.insert(leadCollectionHistory).values(data);
  return result;
}

export async function getLeadCollectionHistoryList(limit = 50) {
  const db = await getDb();
  if (!db) return [];
  const rows = await db
    .select()
    .from(leadCollectionHistory)
    .orderBy(desc(leadCollectionHistory.executedAt))
    .limit(limit);
  return rows;
}

export async function getLeadCollectionHistoryByBatchId(batchId: string) {
  const db = await getDb();
  if (!db) return null;
  const rows = await db
    .select()
    .from(leadCollectionHistory)
    .where(eq(leadCollectionHistory.batchId, batchId))
    .limit(1);
  return rows[0] || null;
}
...
export async function getSalesEmailLogs(options?: {
  ...
  if (options?.search) {
    ...
    conditions.push(
      or(
        like(salesEmailLogs.toEmail, searchTerm),
        like(salesEmailLogs.toName, searchTerm),
        like(salesEmailLogs.toCompany, searchTerm),
        like(salesEmailLogs.subject, searchTerm)
      )
    );
  }
  if (options?.sendType) {
...
export async function getSalesEmailStats() {
  ...
  const byType = await db
    ...
    .from(salesEmailLogs)
    .groupBy(salesEmailLogs.sendType);
  ...
  const [recentResult] = await db
    .select({ count: sql<number>`count(*)` })
    .from(salesEmailLogs)
    .where(gte(salesEmailLogs.sentAt, sevenDaysAgo));
