  ...
  snapshot:publicProcedure.input(z.object({key:z.string().min(1)})).query(async({input})=>{
    ...
    const [memberIdentity,people,staffLinks,memberLinks,linePoints,userPoints,products,productLinks,ordersReceipts,inventory,columns,recoveryHealth]=await Promise.all([
      ...
      first(`SELECT
        ...
        SUM(status='draft') AS draftProducts,
        SUM(status='sold_out') AS soldOutProducts,
        SUM(status='archived') AS archivedProducts,
        SUM(price<=0) AS invalidPriceProducts,
        SUM(stock<0) AS negativeStockProducts,
        SUM(status='active' AND (imageUrl IS NULL OR imageUrl='') AND JSON_LENGTH(COALESCE(imageUrls,JSON_ARRAY()))=0) AS activeMissingImages,
        SUM(status='active' AND stock<=0) AS activeZeroStock,
        (SELECT COUNT(*) FROM mall_product_variants) AS variantRows,
        (SELECT COUNT(*) FROM mall_product_variants WHERE isActive='yes') AS activeVariants,
        (SELECT COUNT(*) FROM brand_products) AS brandProductRows,
        (SELECT COUNT(*) FROM product_master) AS productMasterRows,
        (SELECT COUNT(*) FROM receipt_products) AS receiptProductRows`),
      first(`SELECT
