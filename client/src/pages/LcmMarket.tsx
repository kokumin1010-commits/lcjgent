      ...
      <main>
          ...
          <div className="mx-auto grid max-w-[1440px] gap-10 px-5 py-16 md:grid-cols-[1.05fr_.95fr] md:px-8 md:py-24">
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link href="/lcm/creators" className="inline-flex items-center border border-black bg-white px-5 py-3 text-sm font-black"><Users className="mr-2 h-4 w-4" />ライブコマーサーを探す</Link>
        ...
        <section className="mx-auto max-w-[1440px] px-5 py-16 md:px-8 md:py-20">
          <div className="grid gap-px bg-black/15 md:grid-cols-3">
            {[{ icon: Building2, title: "ブランドが自分で更新", text: "商品追加や情報更新を積み重ね、検索される公式ページへ。" }, { icon: Users, title: "ライブコマーサーを公式ページから探す", text: "本人同意・運営確認済みの得意分野と配信形式から候補を検索。", href: "/lcm/creators" }, { icon: ShieldCheck, title: "卸条件は会員限定", text: "価格、最小発注数、送料、支払条件を承認会員だけに公開。" }].map(({ icon: Icon, title, text, href }) => <article key={title} className="bg-white p-7"><Icon className="h-7 w-7 text-[#d45b16]" /><h3 className="mt-5 text-xl font-black">{title}</h3><p className="mt-3 text-sm font-medium leading-7 text-black/60">{text}</p>{href && <Link href={href} className="mt-5 inline-flex items-center border-b border-black pb-1 text-xs font-black">一覧を見る<ArrowRight className="ml-1 h-4 w-4" /></Link>}</article>)}
          </div>
        </section>
      </main>
    </LcmPublicLayout>
