...
新增并登记`0163_task_completion_acceptance`：新任务及既有未完成任务默认要求验收，仅迁移时已完成的历史任务保留兼容；历史任务重新打开后自动进入新验收规则。Railway启动前迁移只执行尚未登记的版本，已登记版本仅做Schema健康验证。验收事件对外没有UPDATE/DELETE接口，并以完成版本和请求ID唯一约束保持追加式记录；支持触发器的MySQL还会启用数据库层禁止UPDATE/DELETE保护，TiDB或受限数据库账号不支持触发器时则不会阻断服务启动。迁移和运行时升级均具备多实例幂等保护。
...
验证：任务验收、Feed/分组、日报复核、绩效、0161→0163启动迁移、早会媒体/大音频/处理恢复/重试/质量与语音接口等最终专项13文件85项全部通过（前两轮广覆盖16文件97项、14文件86项亦通过）；任务/早会/品牌BD前后端esbuild通过；8GB production build成功；全量TypeScript仍为仓库既有baseline错误，本次目标文件无新增诊断；`git diff --check`、迁移日志JSON、启动脚本语法、直接完成绕过和正式早会browser fallback扫描均通过。多轮独立只读复审发现的迁移口径、触发器重放、浏览器字幕、混合错误分类、品牌BD绕过及日报自动复核幂等ID列宽问题均已逐项修复；正式迁移、ORM与运行时升级的`completionRequestId`已统一为VARCHAR(128)。

首次release `0c784309`的GitHub CI成功，但Railway在启动迁移阶段失败，旧生产版本继续正常提供`/master/tasks`、`/master/morning-meeting`和health HTTP 200。由于当前连接器无Railway日志读取权限，结合启动链范围和仓库既有TiDB兼容约束，将新增加的`CREATE TRIGGER`硬依赖确定为唯一数据库特有风险点。修复保持已经推送的0163 SQL原文与哈希不变，避免失败部署已部分执行后产生ledger hash mismatch；启动器只在当前语句确为`CREATE TRIGGER`且数据库明确返回unsupported/denied时跳过，其他SQL错误仍失败关闭，运行时也采用相同可选增强。隔离MariaDB分别以完整权限和无TRIGGER权限执行0161→0163两次均成功；受限账号真实返回`ER_TABLEACCESS_DENIED_ERROR`时成功降级、必需列/唯一索引仍完整、触发器数量为0。
