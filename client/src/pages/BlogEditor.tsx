...
function AIGenerateDialog({
  ...
  const handleInsertWithSeo = async () => {
    // Use the meta already generated by generateArticle (includes title, SEO, category, tags)
    if (generatedMeta) {
      onInsert(generatedHtml, generatedMeta);
    } else {
      // Fallback: try separate SEO generation
      try {
...
export default function BlogEditor() {
  ...
  // Create tag mutation for auto-creating new tags
  const createTagMutation = trpc.blog.createTag.useMutation();
  const createCategoryMutation = trpc.blog.createCategory.useMutation();

  // Handle AI generated content insertion — fills ALL fields automatically
  const handleAIInsert = useCallback(
    async (html: string, seoData?: any) => {
      if (!editor) return;
      ...
      // 3. Category auto-set
      if (seoData?.suggestedCategoryId && seoData.suggestedCategoryId > 0) {
        setCategoryId(seoData.suggestedCategoryId);
      } else if (seoData?.suggestedCategoryName) {
        // Create new category if suggested
        try {
          const newCat = await createCategoryMutation.mutateAsync({
            name: seoData.suggestedCategoryName,
            slug: seoData.suggestedCategoryName.toLowerCase().replace(/[^a-z0-9\u3040-\u309f\u30a0-\u30ff\u4e00-\u9faf]+/g, "-").replace(/^-|-$/g, ""),
          });
          if (newCat?.id) {
          ...
          // Ignore category creation errors
        }
      }

      // 4. Tags auto-set
      const tagIds: number[] = [];
      if (seoData?.suggestedTagIds?.length) {
        tagIds.push(...seoData.suggestedTagIds);
      }
      // Create new tags if suggested
      if (seoData?.suggestedNewTags?.length) {
        for (const tagName of seoData.suggestedNewTags) {
          try {
            const newTag = await createTagMutation.mutateAsync({
              name: tagName,
              slug: tagName.toLowerCase().replace(/[^a-z0-9\u3040-\u309f\u30a0-\u30ff\u4e00-\u9faf]+/g, "-").replace(/^-|-$/g, ""),
            });
      ...
      toast.success("AI生成記事を全フィールドに自動入力しました");

      // 5. Auto-generate cover image
      const currentTitle = seoData?.title || seoData?.seoTitle || title;
      const keywordsForImage = seoData?.suggestedNewTags || seoData?.keywords || [];
      if (currentTitle && !coverImageUrl) {
        toast.info("カバー画像をAIで自動生成中...");
        setGeneratingCoverImage(true);
        coverImageMutation.mutateAsync({
          title: currentTitle,
          keywords: keywordsForImage,
      ...
      // 6. Auto-generate inline images
      toast.info("本文内のAI画像も自動生成中...");
    ...
    [editor, title, coverImageUrl, coverImageStyle, articleId, coverImageMutation, inlineImageMutation, inlineImageCount, inlineImageStyle, createTagMutation, createCategoryMutation, utils]
  );
