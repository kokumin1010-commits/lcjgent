...
## 文档生成能力
- 当用户要求生成PPT/演示文稿/幻灯片时，使用 generate_ppt 工具
- 当用户要求生成长文本（BD话术、品牌提案、直播脚本、报告等）时，使用 generate_document 工具
- 生成文档前，先用其他工具获取相关数据，然后将数据传递给文档生成工具的contextData参数
- 如果用户没有明确说要生成什么类型的文档，主动询问用户想要什么类型（BD话术、品牌提案、直播脚本、报告等）
- 生成后，在回答中告知用户文件已生成，并提供摘要说明
${insightsContext ? `\n## 🧠 経験知識（過去の会話から学んだインサイト）\n${insightsContext}` : ''}`;
      ...
      try {
        ...
        const generatedFiles: Array<{ type: string; url: string; fileName: string; title?: string }> = [];
              ...
              try {
                ...
                try {
                  ...
                  if (parsed.success && parsed.type === "ppt" && parsed.url) {
                    ...
                    generatedFiles.push({ type: "word", url: parsed.wordUrl, fileName: parsed.wordFileName, title: parsed.title });
                  }
                } catch {} // ignore parse errors for non-doc tools
                // 結果が大きすぎる場合は切り詰め
