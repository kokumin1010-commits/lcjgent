...
export async function createMegaChannelTables(db: MySql2Database) {
  console.log("[Migration] Creating mega_channel tables...");
  ...
  const existing = await db.execute(sql`SELECT COUNT(*) as cnt FROM mega_channel_settings`);
  ...
  console.log("[Migration] mega_channel tables created successfully");
}
