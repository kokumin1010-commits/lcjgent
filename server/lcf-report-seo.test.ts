...
const exhibitors = readFileSync("client/src/pages/Lcf2026Exhibitors.tsx", "utf8");
...
describe("LCF public archive SEO", () => {
  ...
  it("prerenders the complete exhibitor catalogue with collection and breadcrumb structured data", () => {
    ...
    expect(exhibitors).toContain('canonicalPath: catalogPath');
    expect(exhibitors).toContain('"@type": "CollectionPage"');
  });
  ...
  it("uses page-level canonical, OGP and JSON-LD in the SPA", () => {
    ...
    expect(exhibitors).toContain("applyPageSeo");
  });
