...
describe("brand BD command center", () => {
  ...
  it("keeps Railway healthy while the protected brand upgrade runs after listen", () => {
    const index = source("server/_core/index.ts");
    const listenAt = index.indexOf("server.listen(port");
    const backgroundUpgradeAt = index.indexOf(
      "const initializeBrandBusinessStorage"
    );
    expect(listenAt).toBeGreaterThan(0);
    expect(backgroundUpgradeAt).toBeGreaterThan(listenAt);
    expect(index.slice(0, listenAt)).not.toContain(
      "await startBrandBusinessUpgradeSetup()"
    );
    expect(index).toContain("BRAND_BUSINESS_V2_UPGRADE_FAILED");
    expect(index).toContain("related routes remain fail-closed");
    expect(index).toContain("/api/health/brand-bd-command");
    expect(index).toContain('schemaVersion: "brand-business-v2"');
  });
