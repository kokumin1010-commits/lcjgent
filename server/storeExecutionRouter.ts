...
function actor(ctx: any) {
  return { id: Number(ctx?.user?.id || 0) || null, name: String(ctx?.user?.name || ctx?.user?.email || ctx?.user?.openId || 'authenticated-user').slice(0,255) };
}

async function assertStore(connection: any, storeId: number) {
  const [rows] = await connection.query('SELECT id,name,operatorId,operatorName,operator2Id,operator2Name FROM managed_stores WHERE id=? AND isActive=1 LIMIT 1',[storeId]);
  const row = (rows as any[])[0];
  if (!row) throw new TRPCError({ code:'NOT_FOUND', message:'店铺不存在或已归档' });
  return row;
}

async function assertActiveStaff(connection: any, staffId: number) {
  const [rows] = await connection.query(`SELECT id,name FROM staff WHERE id=? AND isActive='active' AND archivedAt IS NULL AND mergedIntoStaffId IS NULL LIMIT 1 FOR UPDATE`,[staffId]);
  const row = (rows as any[])[0];
  if (!row) throw new TRPCError({ code:'BAD_REQUEST', message:'日报提交人不存在或已停用，请重新选择在职员工' });
  return { id:Number(row.id),name:String(row.name) };
}

async function writeAudit(connection: any, input: { storeId:number; entityType:'goal_cycle'|'goal'|'work_item'|'report'|'review'; entityId?:number|null; seriesKey?:string|null; action:string; before?:unknown; after?:unknown; reason?:string|null; ctx:any }) {
  const a = actor(input.ctx);
  await connection.query(`INSERT INTO store_execution_audit_logs (storeId,entityType,entityId,seriesKey,action,beforeJson,afterJson,actorId,actorName,reason) VALUES (?,?,?,?,?,?,?,?,?,?)`,[
    input.storeId,input.entityType,input.entityId||null,input.seriesKey||null,input.action,input.before?JSON.stringify(input.before):null,input.after?JSON.stringify(input.after):null,a.id,a.name,input.reason||null,
  ]);
}

export function calculateGoalAchievement(direction:'increase'|'decrease'|'maintain',target:number,actual:number|null):number|null {
  if (actual===null || !Number.isFinite(actual) || !Number.isFinite(target)) return null;
  if (direction==='maintain') return Math.max(0,100-(Math.abs(actual-target)/Math.max(Math.abs(target),1))*100);
...
export function calculateDailyCompliance(input: {
  ...
  today: string;
  requiredFrom?: string;
  reports: Array<{ id?: number; seriesKey?: string; periodStart: unknown; status: string; title?: string; createdAt?: unknown; createdByName?: string; submitterStaffId?: number|null; submitterName?: string|null }>;
}) {
  ...
  for (const report of input.reports) {
    ...
    reportsByDate.set(date,dateReports);
  ...
  const todayReports = reportsByDate.get(input.today) || [];
  ...
  const calendar = expectedDates.map(date => {
    const dateReports=reportsByDate.get(date)||[];
    const representative=reportByDate.get(date)||null;
    return {
      date,
      status: submittedStatuses.has(String(representative?.status || '')) ? 'submitted' : representative?.status === 'draft' ? 'draft' : date === input.today ? 'pending' : 'missing',
      report:representative,
      reports:dateReports,
      reportCount:dateReports.length,
      submitterNames:Array.from(new Set(dateReports.map(report=>report.submitterName||report.createdByName).filter(Boolean))),
    };
  });
  return {
    today: input.today,
...
export function deterministicDailySeriesKey(storeId: number, date: string, submitterStaffId: number): string {
  const chars = createHash('sha256').update(`lcj-store-daily:${storeId}:${date}:staff:${submitterStaffId}`).digest('hex').slice(0, 32).split('');
  chars[12] = '5';
...
async function getDailyReportRows(connection: any, storeId: number, year: number, month: number) {
  ...
  const [legacyResult, masterResult] = await Promise.all([
    connection.query(
      `SELECT id,seriesKey,storeId,periodStart,periodEnd,title,status,workSummary,highlights,issuesRisks,actionsTaken,nextPlan,supportNeeded,activityJson,evidenceJson,kpiSnapshotJson,dataEvidenceJson,linkedCycleId,submitterStaffId,submitterName,versionNumber,createdById,createdByName,confirmedByName,confirmedAt,createdAt
  ...
  const legacyRows = legacyResult[0] as any[];
  const masterRows = (masterResult[0] as any[]).map(row => {
    const payload = typeof row.payloadJson === 'string' ? JSON.parse(row.payloadJson) : row.payloadJson || {};
    return {
      id:`master:${row.id}`,
      seriesKey:`master:${row.id}`,
      storeId:row.storeId,
      periodStart:row.reportDate,
      periodEnd:row.reportDate,
      title:'协作式店长日报',
      status:'submitted',
      workSummary:Array.isArray(payload?.execution?.completedItems) ? payload.execution.completedItems.join('\n') : '',
      highlights:'',
      issuesRisks:String(payload?.execution?.issuesRisks || ''),
      actionsTaken:String(payload?.execution?.actionsTaken || ''),
      nextPlan:Array.isArray(payload?.execution?.tomorrowItems) ? payload.execution.tomorrowItems.map((item:any)=>item?.title).filter(Boolean).join('\n') : '',
      supportNeeded:Array.isArray(payload?.execution?.supportItems) ? payload.execution.supportItems.map((item:any)=>item?.title).filter(Boolean).join('\n') : '',
      activityJson:JSON.stringify(payload?.content || {}),
      evidenceJson:'[]',
      kpiSnapshotJson:JSON.stringify(payload?.core || {}),
      dataEvidenceJson:JSON.stringify(payload?.metricMeta || {}),
      linkedCycleId:null,
      submitterStaffId:null,
      submitterName:row.updatedByName || row.createdByName || null,
      versionNumber:row.versionNumber,
      createdById:row.updatedById || row.createdById || null,
      createdByName:row.updatedByName || row.createdByName || null,
      confirmedByName:null,
      confirmedAt:null,
      createdAt:row.updatedAt || row.createdAt,
    };
  });
  return [...legacyRows, ...masterRows]
    .sort((left,right) => String(right.periodStart).localeCompare(String(left.periodStart)) || String(right.id).localeCompare(String(left.id)))
    .map(row => ({
      ...row,
...
function dateOnly(value: unknown) {
  const text = value instanceof Date ? value.toISOString().slice(0,10) : String(value ?? '').trim().slice(0,10).replace(/\//g,'-');
  return /^\d{4}-\d{2}-\d{2}$/.test(text) ? text : null;
}
function numberValue(value: unknown) {
  if (value && typeof value==='object' && 'value' in value) return numberValue((value as any).value);
  const parsed = Number(String(value ?? '').replace(/[¥￥,%\s,]/g,''));
  return Number.isFinite(parsed) ? parsed : 0;
}
function pick(row: Record<string,any>, keys: string[]) {
  for (const key of keys) if (Object.prototype.hasOwnProperty.call(row,key)) return numberValue(row[key]);
  const normalized = Object.keys(row).find(key => keys.some(candidate => key.toLowerCase().includes(candidate.toLowerCase())));
  return normalized ? numberValue(row[normalized]) : 0;
}

const METRIC_KEYS: Record<string,string[]> = {
  gmv:['GMV','売上','销售额','Gross revenue'],
  orders:['注文','注文数','订单数','Orders'],
  customers:['カスタマー数','顧客数','客户数','Buyers'],
  refundAmount:['返金','退款金額','退款金额','返品金額','キャンセル金額','Refund'],
  adSpend:['広告費','广告消耗','広告消耗','Cost','Spend'],
  impressions:['表示回数','曝光量','Impressions'],
...
async function cloneReportVersion(connection:any, before:any, input:{status:string; actor:{id:number|null;name:string}; deleted?:boolean}) {
  await connection.query('UPDATE store_operation_reports SET isCurrent=0 WHERE id=?',[before.id]);
  const [versions]=await connection.query('SELECT COALESCE(MAX(versionNumber),0) AS maxVersion FROM store_operation_reports WHERE seriesKey=?',[before.seriesKey]);
  const version=Number((versions as any[])[0]?.maxVersion||0)+1;
  const [result]=await connection.query(`INSERT INTO store_operation_reports
    (seriesKey,storeId,reportType,periodStart,periodEnd,title,status,workSummary,highlights,issuesRisks,actionsTaken,nextPlan,supportNeeded,tagsJson,activityJson,evidenceJson,kpiSnapshotJson,dataEvidenceJson,linkedCycleId,submitterStaffId,submitterName,versionNumber,isCurrent,supersedesId,createdById,createdByName,confirmedById,confirmedByName,confirmedAt,deletedAt,deletedById,deletedByName)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,IF(?=1,CURRENT_TIMESTAMP,NULL),?,?)`,[
    before.seriesKey,before.storeId,before.reportType,before.periodStart,before.periodEnd,before.title,input.status,before.workSummary,before.highlights,before.issuesRisks,before.actionsTaken,before.nextPlan,before.supportNeeded,before.tagsJson,before.activityJson,before.evidenceJson,before.kpiSnapshotJson,before.dataEvidenceJson,before.linkedCycleId,before.submitterStaffId||null,before.submitterName||null,version,before.id,input.actor.id,input.actor.name,
    input.status==='confirmed'?input.actor.id:before.confirmedById,input.status==='confirmed'?input.actor.name:before.confirmedByName,input.status==='confirmed'?new Date():before.confirmedAt,input.deleted?1:0,input.deleted?input.actor.id:null,input.deleted?input.actor.name:null,
  ]);
  const id=Number((result as any).insertId);
  const [rows]=await connection.query('SELECT * FROM store_operation_reports WHERE id=?',[id]);
...
export const storeExecutionRouter = router({
  ...
  kpiPreview: protectedProcedure.input(z.object({ storeId:z.number().int().positive(),periodStart:z.string().date(),periodEnd:z.string().date() })).query(async({input})=>{
  ...
  dailyCompliance: protectedProcedure.input(z.object({storeId:z.number().int().positive(),year:z.number().int(),month:z.number().int().min(1).max(12)})).query(async({input})=>{
    ...
    const allRows=await getAllDailyReportStatusRows(p,input.storeId,STORE_DAILY_REPORT_REQUIRED_FROM,today);
    const compliance=calculateDailyCompliance({year:input.year,month:input.month,today,reports});
    return {...compliance,consecutiveMissingDays:calculateConsecutiveMissingDays({today,reports:allRows}),reports};
  }),
  ...
  saveCycle: protectedProcedure.input(z.object({id:z.number().int().positive().optional(),storeId:z.number().int().positive(),cycleType:z.enum(['three_month','monthly','custom']),title:z.string().min(1).max(500),periodStart:z.string().date(),periodEnd:z.string().date(),managerStaffId:z.number().int().positive().nullable().optional(),managerName:z.string().min(1).max(255),notes:nullableText})).mutation(async({input,ctx})=>{
    if(input.periodEnd<input.periodStart) throw new TRPCError({code:'BAD_REQUEST',message:'结束日期不能早于开始日期'}); const p=await pool(); const c=await p.getConnection(); const a=actor(ctx); try{await c.beginTransaction();await assertStore(c,input.storeId);let id=input.id;let before=null;if(id){const [rows]=await c.query('SELECT * FROM store_manager_goal_cycles WHERE id=? AND storeId=? AND deletedAt IS NULL FOR UPDATE',[id,input.storeId]);before=(rows as any[])[0];if(!before)throw new TRPCError({code:'NOT_FOUND',message:'目标周期不存在'});await c.query(`UPDATE store_manager_goal_cycles SET cycleType=?,title=?,periodStart=?,periodEnd=?,managerStaffId=?,managerName=?,notes=?,updatedById=?,updatedByName=? WHERE id=?`,[input.cycleType,input.title,input.periodStart,input.periodEnd,input.managerStaffId||null,input.managerName,input.notes||null,a.id,a.name,id]);}else{const [r]=await c.query(`INSERT INTO store_manager_goal_cycles (storeId,managerStaffId,managerName,cycleType,title,periodStart,periodEnd,notes,createdById,createdByName,updatedById,updatedByName) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,[input.storeId,input.managerStaffId||null,input.managerName,input.cycleType,input.title,input.periodStart,input.periodEnd,input.notes||null,a.id,a.name,a.id,a.name]);id=Number((r as any).insertId);}const [afterRows]=await c.query('SELECT * FROM store_manager_goal_cycles WHERE id=?',[id]);const after=(afterRows as any[])[0];await writeAudit(c,{storeId:input.storeId,entityType:'goal_cycle',entityId:id,action:before?'cycle_updated':'cycle_created',before,after,ctx});await c.commit();return after;}catch(e){await c.rollback();throw e;}finally{c.release();}
  }),
  activateCycle: adminProcedure.input(z.object({id:z.number().int().positive(),reason:z.string().min(3).max(1000)})).mutation(async({input,ctx})=>{const p=await pool();const c=await p.getConnection();const a=actor(ctx);try{await c.beginTransaction();const [rows]=await c.query('SELECT * FROM store_manager_goal_cycles WHERE id=? AND deletedAt IS NULL FOR UPDATE',[input.id]);const before=(rows as any[])[0];if(!before)throw new TRPCError({code:'NOT_FOUND'});await c.query(`UPDATE store_manager_goal_cycles SET status='active',activatedById=?,activatedByName=?,activatedAt=CURRENT_TIMESTAMP,updatedById=?,updatedByName=? WHERE id=?`,[a.id,a.name,a.id,a.name,input.id]);const after={...before,status:'active'};await writeAudit(c,{storeId:Number(before.storeId),entityType:'goal_cycle',entityId:input.id,action:'cycle_activated',before,after,reason:input.reason,ctx});await c.commit();return{success:true};}catch(e){await c.rollback();throw e;}finally{c.release();}}),
  listGoals: protectedProcedure.input(z.object({cycleId:z.number().int().positive()})).query(async({input})=>{const [rows]=await (await pool()).query('SELECT * FROM store_manager_goals WHERE cycleId=? AND deletedAt IS NULL ORDER BY sortOrder,id',[input.cycleId]);return rows as any[];}),
  ...
  dailyCheckIn: protectedProcedure.input(z.object({
    ...
    submitterStaffId:z.number().int().positive(),
    ...
    try {
      ...
      const submitter=await assertActiveStaff(c,input.submitterStaffId);
      const [currentRows]=await c.query(`SELECT * FROM store_operation_reports WHERE storeId=? AND reportType='daily' AND periodStart=? AND submitterStaffId=? AND isCurrent=1 AND deletedAt IS NULL ORDER BY id DESC LIMIT 1 FOR UPDATE`,[input.storeId,input.reportDate,submitter.id]);
      const before=(currentRows as any[])[0]||null;
      const seriesKey=before?String(before.seriesKey):deterministicDailySeriesKey(input.storeId,input.reportDate,submitter.id);
      ...
      const title=`${String(store.name)} ${input.reportDate} ${submitter.name} 日报`;
      const [result]=await c.query(`INSERT INTO store_operation_reports (seriesKey,storeId,reportType,periodStart,periodEnd,title,status,workSummary,highlights,issuesRisks,actionsTaken,nextPlan,supportNeeded,tagsJson,activityJson,evidenceJson,kpiSnapshotJson,dataEvidenceJson,linkedCycleId,submitterStaffId,submitterName,versionNumber,isCurrent,supersedesId,createdById,createdByName) VALUES (?,?,'daily',?,?,?,'submitted',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?,?)`,[
        seriesKey,input.storeId,input.reportDate,input.reportDate,title,input.workSummary,input.highlights,input.issuesRisks||null,input.actionsTaken||null,input.nextPlan,input.supportNeeded||null,JSON.stringify([]),JSON.stringify(input.activity),JSON.stringify(input.evidence),JSON.stringify(snapshot.metrics),JSON.stringify({hasSourceData:snapshot.hasSourceData,sourceRows:snapshot.sourceRows,evidence:snapshot.evidence}),input.linkedCycleId||null,submitter.id,submitter.name,version,before?Number(before.id):null,a.id,a.name,
      ]);
      ...
      await c.commit();
      return {id:reportId,seriesKey,versionNumber:version,status:'submitted' as const,reportDate:input.reportDate,submitterStaffId:submitter.id,submitterName:submitter.name,updatedWorkItems:input.workUpdates.length,kpiSnapshot:snapshot.metrics,dataEvidence:{hasSourceData:snapshot.hasSourceData,sourceRows:snapshot.sourceRows,evidence:snapshot.evidence}};
