    ...
    <section className="mt-8 grid gap-px bg-black/15 lg:grid-cols-[300px_1fr]">
      ...
      <div className="bg-[#f6f4ee] p-5 md:p-8">
        ...
        {activeBrand && !showBrandEditor && <>
          {selectedBrand?.member.status === "pending" && <div className="mb-5 flex gap-3 border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><Clock3 className="mt-0.5 h-5 w-5 shrink-0" /><span><strong>仮連携中です。</strong> ブランド情報と商品を非公開の下書きとして編集できます。公開審査への提出、サンプル・卸商談への対応は、運営の正式承認後に利用できます。</span></div>}
          <div className="flex flex-wrap items-start justify-between gap-4"><div><span className="inline-flex bg-white px-2.5 py-1 text-[11px] font-black">{selectedBrand?.member.status === "pending" ? "仮連携中" : statusLabels[activeBrand.status] || activeBrand.status}</span><h2 className="mt-3 text-3xl font-black">{activeBrand.displayName}</h2><p className="mt-2 text-sm text-black/55">{activeBrand.tagline || "ブランド情報を追加してください"}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={!canDraftManage} onClick={() => setShowBrandEditor(true)} className="border border-black bg-white px-4 py-3 text-sm font-black disabled:opacity-40">編集する</button>{activeBrand.status === "published" && <Link href={`/lcm/brands/${activeBrand.slug}`} className="inline-flex border border-black bg-white px-4 py-3 text-sm font-black">公開ページ</Link>}{["draft", "rejected"].includes(activeBrand.status) && <button type="button" disabled={submitBrand.isPending || !canPerformApprovedOperations} onClick={() => submitBrand.mutate({ brandId: activeBrand.id })} className="bg-[#171714] px-4 py-3 text-sm font-black text-white disabled:opacity-40">{canPerformApprovedOperations ? "運営確認へ提出" : "正式承認後に提出可能"}</button>}</div></div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-3 border-b border-black/20 pb-4"><div><p className="text-xs font-black tracking-[0.16em] text-black/45">PRODUCTS</p><h3 className="mt-1 text-2xl font-black">商品管理</h3></div><div className="flex gap-2">{canPerformApprovedOperations && <Link href={`/lcm/manage?brand=${activeBrand.id}&requests=1`} className="border border-black bg-white px-4 py-3 text-sm font-black">サンプル・商談</Link>}<button type="button" disabled={!canDraftManage} onClick={() => { setEditingProductId(null); setProductForm(emptyProduct); setShowProductEditor(true); }} className="inline-flex items-center bg-[#f7cc35] px-4 py-3 text-sm font-black disabled:opacity-40"><PackagePlus className="mr-2 h-4 w-4" />商品を追加</button></div></div>
          {showProductEditor ? <ProductEditor form={productForm} setForm={setProductForm} editing={Boolean(editingProductId)} pending={createProduct.isPending || updateProduct.isPending || uploadImage.isPending} onUpload={async (file) => { const base64Data = await fileToBase64(file); const result = await uploadImage.mutateAsync({ brandId: activeBrand.id, fileName: file.name, contentType: file.type as "image/jpeg" | "image/png" | "image/webp", base64Data }); setProductForm((current) => ({ ...current, primaryImageUrl: result.url })); toast.success("商品画像をアップロードしました"); }} onCancel={() => { setShowProductEditor(false); setEditingProductId(null); }} onSave={() => { const data = productPayload(productForm); editingProductId ? updateProduct.mutate({ productId: editingProductId, data }) : createProduct.mutate({ brandId: activeBrand.id, data }); }} /> : <div className="mt-5 grid gap-3">{(manageBrand.data?.products || []).map((item) => <article key={item.id} className="grid gap-4 border border-black/15 bg-white p-4 sm:grid-cols-[88px_1fr_auto] sm:items-center"><div className="aspect-square bg-[#eeeae0]">{item.primaryImageUrl ? <button type="button" onClick={() => setPreviewProductImage({ name: item.name, url: item.primaryImageUrl! })} className="group relative h-full w-full cursor-zoom-in overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9b6200] focus-visible:ring-offset-2" aria-label={`${item.name}のメイン写真を拡大表示`}><img src={item.primaryImageUrl} alt={`${item.name}のメイン写真`} className="h-full w-full object-cover transition-transform duration-200 group-hover:scale-105" /><span aria-hidden="true" className="absolute bottom-1.5 right-1.5 inline-flex items-center bg-black/75 px-1.5 py-1 text-[9px] font-black text-white"><Eye className="mr-1 h-3 w-3" />拡大</span></button> : <div className="grid h-full place-items-center text-[10px] font-bold text-black/35">画像なし</div>}</div><div><div className="flex flex-wrap gap-2"><span className="text-[10px] font-black tracking-[0.12em] text-black/45">{statusLabels[item.status] || item.status}</span>{item.sampleAvailable && <span className="bg-[#dff5ea] px-2 py-0.5 text-[10px] font-black text-[#126445]">サンプル</span>}{item.wholesalePrice && <span className="bg-[#fff0bd] px-2 py-0.5 text-[10px] font-black">卸条件あり</span>}</div><h4 className="mt-1 font-black">{item.name}</h4><p className="mt-1 text-xs text-black/50">{item.summary || "商品説明を追加してください"}</p>{item.rejectionReason && <p className="mt-2 text-xs font-bold text-red-700">要修正：{item.rejectionReason}</p>}</div><div className="flex gap-2 sm:flex-col"><button type="button" onClick={() => { setEditingProductId(item.id); setProductForm(productFormFromItem(item)); setShowProductEditor(true); }} className="border border-black px-3 py-2 text-xs font-black">編集</button>{["draft", "rejected"].includes(item.status) && <button type="button" disabled={!canPerformApprovedOperations} onClick={() => submitProduct.mutate({ productId: item.id })} className="bg-[#171714] px-3 py-2 text-xs font-black text-white disabled:opacity-40">{canPerformApprovedOperations ? "提出" : "正式承認後"}</button>}</div></article>)}{manageBrand.data?.products.length === 0 && <div className="border border-dashed border-black/25 bg-white p-8 text-center text-sm text-black/50">まだ商品がありません。</div>}</div>}
        </>}
      </div>
    </section>
...
function LinkedMembershipQuickStart({ email, accountType, preferredType, companyAccountLink, liverAccountLink, pending, onSubmit }: { email: string; accountType: string; preferredType: "event" | "brand" | "creator" | null; companyAccountLink?: { eligible: true; sourceFestivalApplicationId?: number | null; displayName: string; businessName: string | null } | null; liverAccountLink?: { eligible: true; sourceFestivalApplicationId?: number | null; displayName: string; agencyName: string | null; categories: string[] } | null; pending: boolean; onSubmit: (data: { memberType: "company" | "liver"; displayName: string; businessName?: string; termsAccepted: true }) => void }) {
  ...
  return <LcmPublicLayout><main className="mx-auto max-w-5xl px-5 py-12 md:py-20">
    ...
    <section className="mt-8 border border-black/15 bg-white p-5 md:p-7">
      ...
      <label className="mt-5 flex items-start gap-3 text-xs leading-6"><input type="checkbox" checked={agreed} onChange={(event) => setAgreed(event.target.checked)} className="mt-1" /><span>登録情報をLCMの{memberType === "company" ? "ブランド管理、既存企業との連携申請、商品管理" : "公式プロフィール管理、商品探索、サンプル進行"}に利用することへ同意します。公開情報と編集権限は、本人の提出と運営確認後に反映されます。</span></label>
...
function CatalogLinkDirectory({ initialPage, pending, onClaim, onCreate }: { initialPage: number | null; pending: boolean; onClaim: (page: number, scope: "brand" | "company") => void; onCreate: () => void }) {
  ...
  return <section>
    ...
    <p className="mt-3 max-w-3xl text-sm leading-7 text-black/60">第1回LCFに掲載された会社名・ブランド名・商品名を検索できます。カードをタップすると掲載実績を確認できます。連携するとすぐに非公開の下書き編集を開始でき、公開・申請対応などは運営の正式承認後に利用できます。</p>
