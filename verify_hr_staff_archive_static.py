...
checks = {
    ...
    "archive_requires_inactive_noncurrent_evidence": all(token in texts["archive"] for token in [
        'String(target.staff.isActive) !== "inactive"',
    ...
    "hr_ui_has_archive_and_restore": all(token in texts["hr_ui"] for token in ["アーカイブ箱", "目录から削除", "目录へ復元", "日報・タスク・給与・評価履歴は保持", "canArchiveFromDirectory", "所属未確認"]),
    "hr_default_directory_is_current_active": 'useState("current_active")' in texts["hr_ui"],
    "all_status_cards_are_clickable": all(token in texts["hr_ui"] for token in [
        'showDirectoryCategory("all")',
        'showDirectoryCategory("current_active")',
        'showDirectoryCategory("historical_unknown")',
        'showDirectoryCategory("affiliation_unknown")',
        'showDirectoryCategory("resigned")',
