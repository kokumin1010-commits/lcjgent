...
function BusinessSalesAttributionPanel({ query, yearMonth, onInvalidate }: any) {
  ...
  const availableStores = selectedContract
    ? data.stores.filter((row: any) => (row.brandIds?.length ? row.brandIds : [row.brandId]).map(Number).includes(Number(selectedContract.brandId)))
    : data.stores;
  const selectedStore = data.stores.find((row: any) => String(row.id) === storeId);
  const selectedStoreBrandIds = (selectedStore?.brandIds?.length ? selectedStore.brandIds : (selectedStore?.brandId ? [selectedStore.brandId] : [])).map(Number);
  const needsExplicitBrand = sourceType !== "brand_contract" && selectedStoreBrandIds.length > 1;
  const canSubmit = Boolean(staffId && storeId && sourceId && (!needsExplicitBrand || brandId) && (sourceType === "brand_contract" || (businessDate && Number(amount) > 0 && evidenceReference.trim().length >= 5)));
  const submit = () => createMutation.mutate({
    staffId: Number(staffId),
    storeId: Number(storeId),
    brandId: sourceType === "brand_contract" ? selectedContract?.brandId || null : (brandId ? Number(brandId) : null),
