...
import { LcmCreatorQuickViewDialog, type PublicCreator } from "@/components/lcm/LcmCreatorQuickViewDialog";
...
export default function LcmCreatorDirectory() {
  ...
  useEffect(() => applyPageSeo({
    title: "ライブコマーサーを探す｜LCM ライブコマースマーケット",
    description: "本人の公開同意とLCM運営確認を経たライブコマーサー公式プロフィールを、得意カテゴリ・配信形式・所属から検索できます。",
    canonicalPath: "/lcm/creators",
    image: ogImage,
    jsonLd: [{ "@context": "https://schema.org", "@type": "CollectionPage", name: "LCM ライブコマーサーディレクトリ", url: "https://www.livecommercefestival.com/lcm/creators", description: "公開同意・運営確認済みライブコマーサーの公式プロフィール一覧" }],
  }), []);
  ...
  useEffect(() => {
    ...
    return applyPageSeo({ title: "ライブコマーサーを探す｜LCM ライブコマースマーケット", description: "本人の公開同意とLCM運営確認を経たライブコマーサー公式プロフィールを検索できます。", canonicalPath: "/lcm/creators", image: ogImage, jsonLd: [{ "@context": "https://schema.org", "@type": "ItemList", itemListElement: itemList }] });
  }, [itemList]);
