...
import {
  InfluencerImagePastePicker,
  type InfluencerImageFileSource,
  type InfluencerImagePasteError,
} from "@/components/influencer/InfluencerImagePastePicker";
import {
  AlertTriangle,
  ArrowRight,
  BarChart3,
...
const CREATOR_IMPORT_ACCEPT = "image/jpeg,image/png,image/webp,.xlsx,.xls,.csv,text/csv";
const CHAT_SCREENSHOT_ACCEPT = "image/jpeg,image/png,image/webp";
const CREATOR_IMPORT_MAX_BYTES = 5 * 1024 * 1024;
const CHAT_SCREENSHOT_MAX_BYTES = 10 * 1024 * 1024;
const CHAT_SCREENSHOT_MAX_FILES = 10;
...
export default function InfluencerBd() {
  ...
  const selectedCampaignStoreBrandIds = (selectedCampaignStore?.brandIds || []).map(Number);
  const campaignBrandOptions = campaignForm.storeId
    ? (bootstrap.data?.brands || []).filter((brand: any) =>
  ...
  const showCreatorImportPickerError = (error: InfluencerImagePasteError) => {
    const messages: Record<InfluencerImagePasteError, string> = {
      "no-image": L("剪贴板中没有图片，请先复制达人主页截图", "クリップボードに画像がありません。プロフィール画像をコピーしてください"),
      "unsupported-image": L("粘贴图片只支持JPEG、PNG或WEBP", "貼り付け画像はJPEG・PNG・WEBPのみ対応しています"),
      "file-too-large": L("达人截图或表格必须小于5MB", "クリエイター画像・表は5MB未満にしてください"),
      "too-many-files": L("每次只能识别1张截图或1个表格", "1回につき画像または表を1件だけ認識できます"),
    };
    toast.error(messages[error]);
  };

  const handleCreatorImportFiles = async (files: File[], source: InfluencerImageFileSource) => {
    const file = files[0];
    if (!file) return;
    if (source === "paste") {
      toast.success(L("已粘贴达人截图，正在AI识别…", "クリエイター画像を貼り付けました。AI認識中です…"));
    }
    await recognizeCreatorFile(file);
  };

  const showChatScreenshotPickerError = (error: InfluencerImagePasteError) => {
    const messages: Record<InfluencerImagePasteError, string> = {
      "no-image": L("剪贴板中没有聊天截图", "クリップボードにチャット画像がありません"),
      "unsupported-image": L("聊天截图只支持JPEG、PNG或WEBP", "チャット画像はJPEG・PNG・WEBPのみ対応しています"),
      "file-too-large": L("单张聊天截图必须小于10MB", "チャット画像は1枚10MB未満にしてください"),
      "too-many-files": L("聊天截图最多10张", "チャット画像は最大10枚です"),
    };
    toast.error(messages[error]);
  };

  const addPendingScreenshots = (files: File[], source: InfluencerImageFileSource) => {
    setPendingFiles(current => [...current, ...files].slice(0, CHAT_SCREENSHOT_MAX_FILES));
    if (source === "paste") {
      toast.success(L(`已粘贴${files.length}张聊天截图`, `チャット画像を${files.length}枚貼り付けました`));
    }
  };
  ...
  const submitCampaign = async () => {
    if (campaignForm.storeId && !campaignForm.brandId) {
      toast.error(L("该店铺必须明确选择推广品牌", "店舗に紐づくブランドを選択してください"));
      return;
    }
    try {
      ...
      <div className="mx-auto max-w-[1680px] space-y-6 p-4 md:p-6 lg:p-8">
        ...
        <Tabs defaultValue="today" className="space-y-5">
          ...
          <TabsContent value="creators" className="space-y-4">
            <Card className="border-slate-200 shadow-sm"><CardHeader className="gap-4 lg:flex-row lg:items-center lg:justify-between"><div><CardTitle>{L("达人资料库", "クリエイターデータベース")}</CardTitle><CardDescription>{L("负责人、平台、粉丝量、类目和最近联络统一管理。", "担当者、プラットフォーム、フォロワー、カテゴリ、最終連絡を一元管理します。")}</CardDescription></div><div className="flex flex-wrap gap-2">{isAdmin && <Button variant="outline" onClick={() => setCreatorDedupeDialogOpen(true)}><ScanSearch className="mr-2 h-4 w-4" />{L("账号ID查重", "アカウントID重複確認")}</Button>}<InfluencerImagePastePicker accept={CREATOR_IMPORT_ACCEPT} ariaLabel={L("选择截图或表格，或粘贴达人截图", "画像・表を選択、またはクリエイター画像を貼り付け")} className="inline-flex h-10 cursor-pointer items-center justify-center rounded-md border border-indigo-200 bg-indigo-50 px-4 text-sm font-medium text-indigo-700 transition-colors hover:bg-indigo-100" disabled={recognizingCreatorFile} maxFileBytes={CREATOR_IMPORT_MAX_BYTES} maxFiles={1} pasteFilePrefix="influencer-profile" onError={showCreatorImportPickerError} onFiles={handleCreatorImportFiles}>{recognizingCreatorFile ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}{recognizingCreatorFile ? L("正在识别…", "認識中…") : L("选择 / Ctrl+V识别", "選択 / Ctrl+V認識")}</InfluencerImagePastePicker><Button onClick={() => openCreator()}><Plus className="mr-2 h-4 w-4" />{L("新增达人", "クリエイター追加")}</Button></div></CardHeader><CardContent>
              <div className="mb-4">
