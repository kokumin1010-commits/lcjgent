...
export default function LcmCreatorProfile() {
  ...
  useEffect(() => {
    ...
    return applyPageSeo({
      title: `${creator.displayName}｜LCM ライブコマーサー公式ページ`,
  ...
  if (!creator) return <LcmPublicLayout><main className="grid min-h-[70vh] place-items-center px-5 text-center"><div><h1 className="text-3xl font-black">公開プロフィールが見つかりません</h1><p className="mt-3 text-sm text-black/55">公開停止中、または運営確認前の可能性があります。</p><Link href="/lcm/creators" className="mt-6 inline-flex border-b border-black pb-1 text-sm font-black">ライブコマーサー一覧へ戻る</Link></div></main></LcmPublicLayout>;
