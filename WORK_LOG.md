...
## 2026-09-27｜品牌直播条件・KG确认・排班强制门禁（本番反映前）
在`/master/brands/:id`排班区前新增品牌直播条件审批面板。商务部或super admin可登记每一场直播的头部达人、TikTok直播账号、助播是否到场、整体机制、佣金、坑位费、开始/结束时间、商品规格、原价、折后价、商品机制、商品佣金、库存，以及无保证／保ROI／保最低GMV／其他保证的数值和具体条款；草稿允许分次补充，但提交KG确认前所有要求项必须明确填写，空值不会静默变成0。

系统只允许super admin配置当前KG确认账号。条件提交后，只有该后台账号可确认或退回并填写意见；商务需按退回意见继续谈判并重新提交。每次创建、编辑、提交、确认、退回、排班、取消和确认账号变更均写不可变审计事件，页面显示录入人、更新人、提交人和确认人。主播必须从活跃主播主数据选择，ID、姓名和已登记TikTok账号三者在条件保存和排班时均由服务端校验；商品ID必须属于当前品牌。

所有品牌排班创建、普通字段更新、改主播／账号／时间／品牌、取消和取消后恢复都统一经过服务端门禁，只有商务或super admin能操作。系统要求品牌、主播ID/姓名/TikTok账号及秒级直播时间与一份尚未使用的KG已确认条件完全一致；每份确认条件只能绑定一次，绑定后成为已排班并保留不可变关联历史。品牌重复排班和批量更新被拒绝，必须每场单独确认；批量取消在单一事务内失活关联并消耗原确认，不能通过取消后恢复重复使用。无品牌的既有普通排班流程保持兼容。

新增6张业务表由Railway production限定的运行时升级创建。升级在HTTP监听后后台执行，完成前品牌审批与品牌排班安全失败关闭；执行DDL前取得命名锁并完成已验证备份，空的部分schema可在备份后重建，非空漂移会停止升级。若发现早期可空`targetLiverId`列，则在备份后锁定空值行，仅按规范化主播姓名＋非空TikTok账号唯一匹配活跃主播；无匹配或多匹配立即回滚，全部成功后才把列收紧为`NOT NULL`并重做严格健康检查。`/api/health/brand-live-approval`提供只读健康状态。不新增依赖或环境变量，不直接写入production业务数据。

验证：品牌审批与公开排班focused 2 files・16 tests通过；品牌业务、品牌数据完整性、展位及审批focused中，审批相关全部通过，后台菜单总数契约存在latest-main既有1项失配；全项目Vitest为4,681通过、45跳过、228失败，失败集中在latest-main既有52个数据库／外部环境／source-contract文件，本次`brandLiveApproval.test.ts`为0失败。production build成功，仅有仓库既有`receiptMaskingService.ts` sharp warning。完整TypeScript保持latest-main既有1,163项诊断，新增品牌审批文件0项。1440px与390px真实构建视觉QA通过，审批卡、展开详情、条件弹窗和商品区均存在，横向溢出0、console/page error 0。四轮独立只读复审最终为GO，P0/P1均为0。验证未连接或写入production数据库。
