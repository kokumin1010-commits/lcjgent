...
def sku_card_input(card, label_text: str):
    label = card.locator("label").filter(has_text=label_text).first
    return label.locator("xpath=following-sibling::input[1]")


def fill_sku(card, *, name: str, price: str, lowest: str = "", discount: str = "", promotion: str = "", sku_code: str = "", stock: str = "0"):
    sku_card_input(card, "名称").fill(name)
    if sku_code:
        sku_card_input(card, "SKU番号").fill(sku_code)
    sku_card_input(card, "定価 (¥)").fill(price)
    if lowest:
        sku_card_input(card, "最低価 (¥)").fill(lowest)
    if discount:
        sku_card_input(card, "折扣率").fill(discount)
    sku_card_input(card, "库存").fill(stock)
    if promotion:
        sku_card_input(card, "促销 / 组合").fill(promotion)
...
with sync_playwright() as playwright:
    ...
    existing_row.locator("td").last.locator("button").first.click()
    ...
    fill_sku(sku_cards.nth(0), name="既存SKU更新", sku_code="EXISTING-SKU-A", price="18000", lowest="3000", promotion="1+1", stock="8")
    edit_dialog.get_by_role("button", name="+ SKU追加").click()
    sku_cards = edit_dialog.locator("div.border-teal-100")
    fill_sku(sku_cards.nth(1), name="追加SKU-B", sku_code="EXISTING-SKU-B", price="24000", lowest="4800", promotion="1+2", stock="12")
    ...
    existing_row.get_by_role("button", name="子SKU 2件を表示").click()
    ...
    sku_card_input(edit_cards.nth(0), "促销 / 组合").fill("1+4")
    edit_dialog.get_by_role("button", name="更新", exact=True).click()
    page.get_by_text("既存商品A 更新", exact=True).wait_for(state="visible", timeout=15_000)
    ...
    existing_row.locator("td").last.locator("button").first.click()
    edit_dialog = page.get_by_role("dialog")
    second_edit_promotion_persisted = sku_card_input(edit_dialog.locator("div.border-teal-100").nth(0), "促销 / 组合").input_value() == "1+4"
    ...
    fill_sku(create_cards.nth(0), name="新規SKU-10個", sku_code="NEW-SKU-10", price="10000", lowest="7000", promotion="1+1", stock="10")
    fill_sku(create_cards.nth(1), name="新規SKU-20個", sku_code="NEW-SKU-20", price="18000", lowest="11000", promotion="1+2", stock="20")
    fill_sku(create_cards.nth(2), name="新規SKU-30個", sku_code="NEW-SKU-30", price="25000", lowest="15000", promotion="1+4", stock="30")
    ...
    created_row = page.get_by_role("row").filter(has_text="新規商品SKUテスト").first
    created_row.get_by_role("button", name="子SKU 3件を表示").click()
    ...
    fill_sku(duplicate_cards.nth(0), name="ＳＫＵ A", sku_code="DUP-A", price="1000")
    fill_sku(duplicate_cards.nth(1), name="sku   a", sku_code="DUP-B", price="1200")
    ...
    relogin_created_row = relogin_page.get_by_role("row").filter(has_text="新規商品SKUテスト").first
    relogin_created_row.get_by_role("button", name="子SKU 3件を表示").click()
