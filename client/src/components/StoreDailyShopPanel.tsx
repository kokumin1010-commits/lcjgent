      ...
      <div className="rounded-xl border bg-white p-4">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
          {[
            ...
            ["广告消耗", money(summary?.adCost), changeText(trend.data?.changes?.adCost)],
        ...
        {trend.isLoading ? (
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-2">
            ...
            {hasAdsRows && <div className="h-80 min-w-0 rounded-xl border p-3"><p className="mb-2 text-sm font-bold text-slate-700">广告消耗 / 广告GMV</p><ResponsiveContainer width="100%" height="90%"><LineChart data={chartRows}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" tick={{fontSize:11}} /><YAxis tick={{fontSize:10}} tickFormatter={(value) => Number(value) >= 10000 ? `${Math.round(Number(value)/10000)}万` : String(value)} /><Tooltip formatter={(value:any) => money(value)} /><Legend /><Line type="monotone" dataKey="adCost" name="广告消耗" stroke="#dc2626" strokeWidth={2} connectNulls={false} /><Line type="monotone" dataKey="adGmv" name="广告GMV" stroke="#059669" strokeWidth={3} connectNulls={false} /></LineChart></ResponsiveContainer></div>}
