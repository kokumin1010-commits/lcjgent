...
export function StoreManagerExecution({store,year,month,staffList,hideLegacyDaily=false}:{store:any;year:number;month:number;staffList:any[];hideLegacyDaily?:boolean}){
  const {user}=useAuth();
  ...
  return <div className="space-y-5">
    ...
    {!hideLegacyDaily&&dailyPickerDate&&<DailyReportPicker reportDate={dailyPickerDate} reports={reportsForDate(dailyPickerDate)} onClose={()=>setDailyPickerDate('')} onOpen={report=>startDailyForm(dailyPickerDate,report)} onCreate={()=>startDailyForm(dailyPickerDate,null)}/>}
    {!hideLegacyDaily&&showDailyForm&&<DailyCheckInForm store={store} reportDate={fillDate} value={editingReport} staffList={staffList} dateReports={reportsForDate(fillDate)} cycles={cycleRows} workItems={workRows} onClose={()=>setShowDailyForm(false)} onSaved={()=>{setShowDailyForm(false);setEditingReport(null);refresh();}}/>}
