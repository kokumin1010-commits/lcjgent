...
生产只读核查确认：两张不同账号提交的原图订单号相同，但其中一条历史OCR结果多插入一位数字。旧系统仅对完全相同的标准化订单号执行硬冲突；一位差异只写入“相似”风险标记，证据自动通过、定时自动审核、前端AI批次、Pass2与部分人工入口仍可能继续批准并发积分。调查与测试记录不保存真实姓名、账号、订单号或图片。
