...
迁移在最新并行提交后顺延为`0157_selection_product_bulk_updates`，幂等创建批量审计表和价格历史表；价格历史软归档列由部署迁移器通过`information_schema`和普通`ADD COLUMN`逐列补齐，不依赖MySQL版本不统一的`ADD COLUMN IF NOT EXISTS`。部署迁移器先执行并行的`0156_bw_member_self_link`关键schema，再执行0157，任一关键schema失败均非零退出，避免应用在审计存储缺失时上线。运行时schema确保同样保留，用于旧环境兼容。

本地验证：选品中心相关28个测试文件158项全部通过；并行Beauty Wallet/账户链路6个文件40项通过、1项既有跳过；production build成功，仅保留仓库既有`receiptMaskingService.ts` sharp namespace warning。全量`pnpm check`因仓库既有1,164条诊断返回2，本次新增的批量服务、RBAC、软归档服务、对话框诊断为0，修改目标的规范化诊断集合相对本次加固前未增加。1280×900与390×844的Mock视觉/交互验收均完成，批量提交成功，对话框边界正常，无console、page或request error。本地测试未访问生产数据库、未修改生产商品、价格、库存或历史记录。

独立只读发布复审初次指出同页旧写入口RBAC绕过、仅更新价格时最低价不同步、0157非通用MySQL语法三项P1；逐项修复并补回归后，第二次复审结论为 **GO，P0/P1阻断0项**。
