...
import { getBrandDayCampaignProfile } from "@shared/brandDayCampaign";
...
export default function BrandDayDetail() {
  ...
  const campaign = getBrandDayCampaignProfile(info.slug);
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8" data-testid="brand-day-detail-page">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <Card className="overflow-hidden border-0 bg-slate-950 text-white shadow-xl transition duration-200 group-hover:-translate-y-0.5 group-hover:shadow-2xl">
            <CardContent className="relative p-6 sm:p-8">
              ...
              <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-amber-300">{info.brandName}</p><h1 className="mt-3 text-2xl font-bold sm:text-4xl">{campaign?.title || info.title}</h1><div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-slate-300"><span className="flex items-center gap-2"><CalendarDays className="h-4 w-4" />{dateTime.format(new Date(info.event_start_at))} 〜 {dateTime.format(new Date(info.event_end_at))} JST</span><Badge className="bg-white/10 text-white">{eventStatusLabel(info.status)}</Badge>{campaign && <Badge className="bg-rose-600 text-white">{campaign.discountLabel}</Badge>}</div></div>
        ...
        <Tabs defaultValue="overview" className="space-y-4">
          ...
          <TabsContent value="overview"><div className="grid gap-4 lg:grid-cols-2"><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="text-base">活動ルール</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-600"><p>タイムゾーン：{info.timezone}</p><p>ランキング最低配信：{info.minimum_stream_minutes}分</p>{campaign && <p>ブランド商品売上賞金：1位 {yen.format(campaign.prizes[0])}／2位 {yen.format(campaign.prizes[1])}／3位 {yen.format(campaign.prizes[2])}</p>}<p>AI画像解析：有効時間は自動反映、時間異常は管理者確認</p><p>旧サイト：{info.legacy_base_url || "—"}</p></CardContent></Card><Card className="border-0 shadow-sm"><CardHeader><CardTitle className="text-base">移行・運用状態</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-600"><p>申込・出場者・成績・商品・画像・監査をeventIdで隔離します。</p><p>確認待ちは承認されるまでランキングに表示されません。</p><p>独立KGDAYサイトは切替完了まで回退用として保持します。</p></CardContent></Card></div></TabsContent>
          ...
          <TabsContent value="ranking"><div className="grid gap-4 lg:grid-cols-2"><Ranking title="ブランド商品売上ランキング" rows={leaderboard.data?.sales || []} value={row => yen.format(row.brandGmv)} prizes={campaign?.prizes} /><Ranking title="配信時間ランキング" rows={leaderboard.data?.streaming || []} value={row => `${Math.floor(row.streamMinutes / 60)}時間${row.streamMinutes % 60}分`} /></div></TabsContent>
