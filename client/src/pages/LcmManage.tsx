...
export default function LcmManage() {
  ...
  const confirmedBrands = (myBrands.data || []).filter((entry) => entry.member.status === "active");
  const selectedBrand = confirmedBrands.find((entry) => entry.brand.id === selectedBrandId)
    || confirmedBrands[0];
  ...
  const claimCatalogSelection = trpc.lcm.claimCatalogSelection.useMutation({ onSuccess: async (data) => { setSelectedBrandId(null); setShowBrandEditor(false); setShowProductEditor(false); toast.success(data.scope === "company" ? `${data.companyName}の管理権限確認を受け付けました。承認後に操作できます` : "ブランド管理権限の確認を受け付けました。承認後に操作できます"); await refresh(); }, onError: (error) => toast.error(error.message) });
  ...
  const unpublishProduct = trpc.lcm.unpublishProduct.useMutation({ onSuccess: async () => { toast.success("商品の公開を停止し、下書きへ戻しました"); await Promise.all([refresh(), utils.lcm.listPublicProducts.invalidate(), utils.lcm.getPublicProduct.invalidate(), utils.lcm.getPublicBrand.invalidate(), utils.lcm.publicStats.invalidate()]); }, onError: (error) => toast.error(error.message) });
...
function BrandDiscoveryPanel({ brands, pending, onAddProduct, onSelectBrand, onClaim, onCreate }: {
  ...
  const confirmedBrands = useMemo(() => brands.filter((entry) => entry.member.status === "active"), [brands]);
  const pendingBrands = useMemo(() => brands.filter((entry) => entry.member.status === "pending"), [brands]);
  const managedMatches = useMemo(() => confirmedBrands.filter((entry) => {
    if (!normalizedQuery) return true;
    return normalizeLcmCatalogName(`${entry.brand.displayName} ${entry.brand.companyName || ""}`).includes(normalizedQuery);
  }), [confirmedBrands, normalizedQuery]);
  const pendingMatches = useMemo(() => pendingBrands.filter((entry) => {
    if (!normalizedQuery) return true;
    return normalizeLcmCatalogName(`${entry.brand.displayName} ${entry.brand.companyName || ""}`).includes(normalizedQuery);
  }), [normalizedQuery, pendingBrands]);
  ...
  const hasMatches = managedMatches.length > 0 || pendingMatches.length > 0 || catalogMatches.length > 0;
  ...
  return <section className="mt-8 border border-black/15 bg-white p-5 md:p-8" aria-labelledby="brand-discovery-heading">
    ...
    {managedMatches.length > 0 && <div className="mt-7"><div className="flex items-center justify-between gap-3"><h3 className="text-lg font-black">管理権限確認済みのブランド</h3><span className="text-xs font-black text-black/40">{managedMatches.length}件</span></div><div className="mt-3 grid gap-3 md:grid-cols-2">{managedMatches.map((entry) => {
      const canAddProduct = entry.brand.status !== "suspended";
      return <article key={entry.brand.id} className="border border-black/15 bg-[#faf8f2] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="inline-flex bg-white px-2 py-1 text-[10px] font-black">{statusLabels[entry.brand.status] || entry.brand.status}</span><h4 className="mt-2 break-words text-xl font-black">{entry.brand.displayName}</h4>{entry.brand.companyName && <p className="mt-1 break-words text-xs text-black/50">{entry.brand.companyName}</p>}</div><CheckCircle2 className="h-5 w-5 shrink-0 text-[#16805b]" /></div><div className="mt-4 grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => onSelectBrand(entry.brand.id)} className="border border-black bg-white px-3 py-3 text-xs font-black">ブランドを確認</button><button type="button" disabled={!canAddProduct} onClick={() => onAddProduct(entry.brand.id)} className="inline-flex items-center justify-center bg-[#f7cc35] px-3 py-3 text-xs font-black disabled:opacity-45"><PackagePlus className="mr-1.5 h-4 w-4" />{entry.brand.status === "suspended" ? "停止中" : "このブランドに商品を追加"}</button></div></article>;
    })}</div></div>}

    {pendingMatches.length > 0 && <div className="mt-7"><div className="flex items-center justify-between gap-3"><h3 className="text-lg font-black">管理権限の確認中</h3><span className="text-xs font-black text-black/40">{pendingMatches.length}件</span></div><div className="mt-3 grid gap-3 md:grid-cols-2">{pendingMatches.map((entry) => <article key={entry.brand.id} className="border border-amber-300 bg-amber-50 p-4"><div className="flex items-start gap-3"><Clock3 className="mt-0.5 h-5 w-5 shrink-0 text-[#9b6200]" /><div><span className="text-[10px] font-black tracking-[0.12em] text-[#9b6200]">運営確認中・操作不可</span><h4 className="mt-2 break-words text-lg font-black">{entry.brand.displayName}</h4>{entry.brand.companyName && <p className="mt-1 break-words text-xs text-black/50">{entry.brand.companyName}</p>}<p className="mt-3 text-xs leading-6 text-black/55">管理権限が確認されるまで、ブランド情報・商品・公開設定は操作できません。</p></div></div></article>)}</div></div>}
