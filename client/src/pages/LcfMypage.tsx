  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        <LcfEditionApplicationCenter />

        <LcfSecondEditionGuide />
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
          ...
          <p className="text-xs leading-6 text-gray-400">この共通アカウントで、LCFの参加申込・QRと、LCMのブランド・商品・配信者プロフィールを切り替えて利用できます。</p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
            ...
            <span style={{ color: "#C9A96E" }}>LIVE配信ブース</span>
          </h3>
          <div className="mb-4 border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100">
            <p className="font-black">第2回の予約ルールは準備中です</p>
            <p className="mt-1 text-xs text-gray-300">第1回と同じ予約・チェックイン方式を使用します。対象ブース、時間枠、予約開始、利用上限などを確定後、このマイページで受付を開始します。</p>
          </div>
          <details className="border border-white/10 bg-black/20 p-4">
            <summary className="cursor-pointer text-sm font-bold text-gray-300">第1回のブース予約履歴を見る</summary>
            <div className="mt-4"><BoothReservationSection historyOnly /></div>
          </details>
        </div>
        )}
...
function BoothReservationSection({ historyOnly = false }: { historyOnly?: boolean }) {
  ...
  const availQuery = trpc.boothReservation.getAllAvailability.useQuery(undefined, { enabled: !historyOnly, refetchInterval: historyOnly ? false : 30_000 });
  ...
  if (historyOnly) {
    if (reservationsQuery.isLoading) return <p className="text-xs text-gray-400">予約履歴を読み込み中...</p>;
    if (!reservations.length) return <p className="text-xs text-gray-500">第1回のブース予約履歴はありません。</p>;
    return (
      <div className="space-y-2">
        {reservations.map((reservation: any) => (
          <div key={reservation.reservationId} className="flex flex-col gap-2 border border-white/10 bg-white/5 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-bold text-white">{reservation.date} {reservation.timeSlot}</p>
              <p className="mt-1 text-xs text-amber-300">ブース {reservation.boothId}｜{reservation.bookingType === "same_day" ? "当日枠" : "事前予約"}</p>
            </div>
            <span className={`w-fit rounded px-2 py-1 text-[10px] ${statusStyle(reservation.status)}`}>{reservation.statusLabel || reservation.status}</span>
          </div>
        ))}
      </div>
    );
