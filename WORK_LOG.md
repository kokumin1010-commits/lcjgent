...
- 修复：员工与部门上传均在multipart正文显式传递UTF-8原文件名；服务端优先使用该字段，并继续兼容旧客户端的Latin-1/Unicode头部名称。另增加受账号层级与部门范围保护的显示文件名纠正字段，原始对象、内容和哈希均不变。
- 验证：HR岗位专项15/15通过，本次修改行TypeScript诊断0件，production build成功。
