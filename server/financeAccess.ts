...
export async function hasFinanceAccess(ctx: TrpcContext): Promise<boolean> {
  ...
  try {
    ...
    return payload.scope === "finance" && Number(payload.userId) === Number(ctx.user.id);
  } catch {
    return false;
  }
}

export async function verifyAndUnlockFinance(ctx: TrpcContext, password: string) {
  ...
  attempts.delete(key);
  const token = await new SignJWT({ scope: "finance", userId: ctx.user.id })
  ...
  return { unlocked: true as const, expiresInSeconds: FINANCE_ACCESS_TTL_SECONDS };
}
