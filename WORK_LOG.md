...
新增并登记`0163_task_completion_acceptance`：新任务及既有未完成任务默认要求验收，仅迁移时已完成的历史任务保留兼容；历史任务重新打开后自动进入新验收规则。Railway启动前迁移只执行尚未登记的版本，已登记版本仅做Schema健康验证；验收事件表禁止UPDATE/DELETE，迁移和运行时升级均具备多实例幂等保护。
...
验证：任务验收、Feed/分组、日报复核、绩效、0161→0163启动迁移、早会媒体/大音频/处理恢复/重试/质量与语音接口等最终专项13文件83项全部通过（前两轮广覆盖16文件97项、14文件86项亦通过）；任务/早会/品牌BD前后端esbuild通过；8GB production build成功；全量TypeScript仍为仓库既有baseline错误，本次目标文件无新增诊断；`git diff --check`、迁移日志JSON、启动脚本语法、直接完成绕过和正式早会browser fallback扫描均通过。多轮独立只读复审发现的迁移口径、触发器重放、浏览器字幕、混合错误分类、品牌BD绕过及日报自动复核幂等ID列宽问题均已逐项修复；正式迁移、ORM与运行时升级的`completionRequestId`已统一为VARCHAR(128)，等待Railway部署。
