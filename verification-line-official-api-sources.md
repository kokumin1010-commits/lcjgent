...
自動ONの配備で古い会話に対する即時一斉送信が発生しないよう、軽量な`line_group_automation_states.autoFollowUpEnabledAt`を追加した。既存active groupのrollout、新規group作成、再招待、OFFからONへの変更時刻を保存し、follow-up候補抽出と送信直前claimの双方で、`lastMessageAt`、`createdAt`、`autoFollowUpEnabledAt`のうち最も新しい日時を無活動期間の起点にする。大きな既存`line_groups` tableへcolumnを追加せず、CREATE-onlyの専用state tableへ分離してdeployment healthを阻害しない。したがって、既存履歴が古くてもON直後から設定日数（既定2日）が経過するまで自動追い候補にはならない。送信時のactive確認、設定再確認、平日9〜18時JST、連携済み・有効なライブコマーサー、最新conversation revision、重複監査、決定的retry key等の既存guardは維持する。通常のgroup投稿への即時AI返信は、設定がONでも連携本人による明示的`@LCJ`／bot self mentionが必須であり、無差別返信には変更していない。
...
検証は自動ON rolloutの単一claim、再起動時no-op、ON時刻によるgrace、candidateと送信直前claimの同一activity anchor、group message一意保存、transaction lock順、設定更新、厳格`@LCJ`返信、一般AI無差別返信停止、lifecycleを含むfocused 9ファイル120件が成功した。最新main統合後のLINE関連全体は35ファイル317件中307件成功し、残る5ファイル10件は本番DB、LINE Login／Messaging API secret・token・APP_URLがローカルにない既存環境依存だった。最終rollout修正はfocused 4ファイル68件、並行selection-center migration contractを含む5ファイル70件が成功した。client Vite build、server esbuild、`pnpm build`は成功し、既存`sharp` warning以外に今回起因のbundle failureはない。build sandboxではDB未接続のためmigrationが`ECONNREFUSED`となったが、既存のfail-closed runtime initializerへ安全に委譲された。全量TypeScriptは既存診断でexit 2だが、今回変更した`LineManagement.tsx`、`lineAiManager.ts`、`groupFollowUpScheduler.ts`、`drizzle/schema.ts`、migration helperおよび`db.ts`／`routers.ts`の変更範囲に新規診断はない。本検証では実LINE送信および本番DBの直接更新を行っていない。

独立release reviewでは、初回にcandidate側と送信直前claim側で`autoFollowUpEnabledAt`を含むactivity anchorが一致しないP1を検出した。候補payloadへ`followUpActivityAt`を追加し、schedulerがその同一値をlocked claimへ渡すよう修正した。さらに、rollout完了前に旧group follow-upおよびlive suggestion、daily ranking、weekly/monthly report、group向けschedule reminderが起動できるP0を検出した。これら全group push schedulerの唯一の起動箇所を`ensureLineGroupAutomationDefaults().then(...)`成功branchへ統合し、失敗時は1つも起動せずretryするfail-closed設計へ変更した。
...
feature SHA `3447dc0b46fb8db6ed3382c72fa59836e642a9b2`はGitHub CIとRailway deploymentがsuccess。後続main SHA `f1cf99ba305fe6cefdf867febf38c1bd4be2878f`へ置換後もfeature commitは祖先として保持され、後続SHAのCI／Railwayもsuccessしている。現在本番をGET-only確認し、`/api/health/line-ai-manager`と`/master/line`はHTTP 200、`aiManagerStorage`／`groupAutomationDefaults`／runtime stateはすべて`ready`。配信asset `LineManagement-D4hJZ4-3.js`と`LineGroupReplyReviewQueue-BRSJHYUl.js`に「グループ対応」「判断に使うグループ会話」「LCJスタッフ」「ユーザータイプ＝スタッフ」「AIおすすめ返信（未送信）」があり、queue assetに`pushMessage`／`replyMessage`はない。実LINE送信、group設定mutation、group leave、本番DB直接操作は実施していない。
