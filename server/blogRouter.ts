...
- keywords: キーワードの配列。各項目は{keyword, searchIntent, difficulty, category}形式
  ...
  // --- Batch Generate Cover Images for articles without cover ---
  batchGenerateCoverImages: protectedProcedure
    .input(z.object({
      limit: z.number().min(1).max(20).default(5).describe("一度に生成する記事数（最大20）"),
    }).optional())
    .mutation(async ({ input }) => {
      ...
      // coverImageUrlがnullの公開記事を取得
      const articlesWithoutCover = await db.select({
        id: blogArticles.id,
      ...
      if (articlesWithoutCover.length === 0) {
        return { generated: 0, total: 0, message: "全記事にカバー画像があります" };
      }
      ...
      for (const article of articlesWithoutCover) {
        try {
          const detectedType = detectArticleType('', article.title);
          const imagePrompt = generateCoverImagePrompt(article.title, detectedType);
          const { url: imageUrl } = await generateImage({ prompt: imagePrompt });
          ...
          if (imageUrl) {
            ...
            const { url: s3Url } = await storagePut(imageKey, imageBuffer, "image/png");
            await updateBlogArticle(article.id, { coverImageUrl: s3Url, coverImageKey: imageKey });
            generated++;
            results.push({ id: article.id, title: article.title, success: true });
          } else {
            results.push({ id: article.id, title: article.title, success: false, error: "No URL returned" });
          }
        } catch (err: any) {
          console.error(`[BatchCover] Failed for article ${article.id}:`, err.message);
          results.push({ id: article.id, title: article.title, success: false, error: err.message });
        }
      }
      
      return { generated, total: articlesWithoutCover.length, results };
    }),
