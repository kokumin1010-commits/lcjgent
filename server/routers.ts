...
- 簡潔に300文字以内`;
  ...
  liverManagement: router({
    ...
    dailySalesTrend: publicProcedure
      .input(z.object({ month: z.string(), agencyId: z.number().nullable().optional() }))
      .query(async ({ input }) => {
...
自然な会話調で、「${liverName}さん」と呼びかけてください。`;
      ...
      getRecentAutoMessages: publicProcedure
        ...
        .query(async ({ input }) => {
          const db = await getDb();
          if (!db) return [];
          const conditions: any[] = [eq(aiCoachMessages.role, 'ai')];
          if (input.messageType && input.messageType !== 'all') {
            conditions.push(eq(aiCoachMessages.messageType, input.messageType));
          }
          const messages = await db
            .select({
              id: aiCoachMessages.id,
              liverId: aiCoachMessages.liverId,
              content: aiCoachMessages.content,
              messageType: aiCoachMessages.messageType,
              contextType: aiCoachMessages.contextType,
              createdAt: aiCoachMessages.createdAt,
            })
            .from(aiCoachMessages)
            .where(and(...conditions))
            .orderBy(desc(aiCoachMessages.createdAt))
          ...
          return messages.map(m => ({
            ...m,
            liverName: liverMap.get(m.liverId) || `Liver #${m.liverId}`,
          }));
        }),
      ...
      getMessageTypeCounts: publicProcedure
        .query(async () => {
          ...
          // Total counts by messageType
          const totalCounts = await db
            .select({
              messageType: aiCoachMessages.messageType,
              total: count(aiCoachMessages.id),
            })
            .from(aiCoachMessages)
            .where(eq(aiCoachMessages.role, 'ai'))
            .groupBy(aiCoachMessages.messageType);
          // Today's counts by messageType
          const todayCounts = await db
            .select({
              messageType: aiCoachMessages.messageType,
              todayCount: count(aiCoachMessages.id),
            })
            .from(aiCoachMessages)
            .where(and(
              eq(aiCoachMessages.role, 'ai'),
              gte(aiCoachMessages.createdAt, today)
            ))
            .groupBy(aiCoachMessages.messageType);
          const todayMap = new Map(todayCounts.map(t => [t.messageType, t.todayCount]));
          return totalCounts.map(t => ({
            messageType: t.messageType,
            total: t.total,
            today: todayMap.get(t.messageType) || 0,
          }));
        }),
      ...
      getDailySendStats: publicProcedure
        .query(async () => {
          ...
          const stats = await db
            .select({
              date: sqlTag<string>`DATE(${aiCoachMessages.createdAt})`,
              totalSent: count(aiCoachMessages.id),
            })
            .from(aiCoachMessages)
            .where(and(
              eq(aiCoachMessages.role, 'ai'),
              gte(aiCoachMessages.createdAt, thirtyDaysAgo)
            ))
            .groupBy(sqlTag`DATE(${aiCoachMessages.createdAt})`)
            .orderBy(sqlTag`DATE(${aiCoachMessages.createdAt})`);
          return stats;
        }),
    }),
