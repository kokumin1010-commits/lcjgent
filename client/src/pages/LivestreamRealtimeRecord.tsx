...
export default function LivestreamRealtimeRecord() {
  ...
  // ペースト（Ctrl+V）で画像アップロード（拍卖モード時はスキップ）
  const isAuctionRef = useRef(isAuction);
  isAuctionRef.current = isAuction;
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      // 拍卖モード時は拍卖専用ハンドラーに任せる
      if (isAuctionRef.current) return;
  ...
  const handleAdd = () => {
    ...
    if (isAuction && (startPrice || finalPrice)) {
      auctionCreateMut.mutate({
        ...
        liverName: livestream?.liver?.name || livestream?.streamerName || livestream?.liverName || "",
