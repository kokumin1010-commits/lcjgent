              ...
              {transactionsLoading ? (
                ...
                <div className="space-y-4">
                  {transactions.map((tx) => {
                    ...
                    const isRecoveryReactivation = tx.type === "earn" && tx.referenceType === "system" && tx.description?.includes("历史恢复余额可用批次重建");
                    const isSystemRecovery = isRecoveryOpening || isRecoveryReactivation;
                    return (
                      <div
                        key={tx.id}
                        className="flex items-center gap-4 p-4 border rounded-lg"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className={isSystemRecovery ? "border-blue-200 bg-blue-50 text-blue-700" : ""}>
                              {isRecoveryOpening ? "システム復旧残高" : isRecoveryReactivation ? "復旧ポイント有効化" : getTransactionLabel(tx.type as TransactionType)}
                            </Badge>
                          </div>
                          ...
                          {isRecoveryReactivation && <div className="text-xs text-blue-700 mt-1">復旧残高を利用可能にした記録です。残高への重複加算はありません</div>}
                        ...
                        {/* Amount */}
                        <div className="text-right flex-shrink-0">
                          <div className={`font-bold ${isSystemRecovery ? "text-blue-600" : tx.amount >= 0 ? "text-green-600" : "text-red-600"}`}>
